  ...
  return (
    <div className="space-y-6">
      {Array.from({ length: rows }).map((_, i) => (
        <div key={i} className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-stretch">
          {/* What We Do */}
          {doValues[i] && (
            <div className="group bg-teal-50/50 rounded-xl p-6 border border-teal-200 hover:bg-teal-50 transition-all duration-300 hover:shadow-lg flex flex-col">
              <div className="flex items-start gap-4">
                <div className="bg-teal-600 rounded-full p-2 mt-1 flex-shrink-0">
                  <Check className="text-white" size={20} />
                </div>
                <div>
                  <h3 className="text-xl font-bold text-slate-900 mb-2">{doValues[i].title}</h3>
                  <p className="text-slate-700 leading-relaxed">{doValues[i].description}</p>
                </div>
              </div>
            </div>
          )}
          {/* What We Don't Do */}
          {dontValues[i] && (
            <div className="group bg-slate-50 rounded-xl p-6 border border-slate-200 hover:bg-slate-100 transition-all duration-300 hover:shadow-lg flex flex-col">
              <div className="flex items-start gap-4">
                <div className="bg-slate-600 rounded-full p-2 mt-1 flex-shrink-0">
                  <X className="text-white" size={20} />
                </div>
                <div>
                  <h3 className="text-xl font-bold text-slate-900 mb-2">{dontValues[i].title}</h3>
                  <p className="text-slate-700 leading-relaxed">{dontValues[i].description}</p>
                </div>
              </div>
            </div>
          )}
